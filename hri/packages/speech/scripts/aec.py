...
class SpeexAEC:
    ...
    def __init__(self, frame_size, filter_length, sample_rate=16000):
        ...
        if speex_lib is None:
            raise RuntimeError("SpeexDSP library not found. Please install it using: ./install_speexdsp.sh")
        ...
        self.speex.speex_echo_ctl(self.state, 24, ctypes.byref(sr))  # SPEEX_ECHO_SET_SAMPLING_RATE
        
        # Initialize preprocessor
        self.prep_state = self.speex.speex_preprocess_state_init(frame_size, sample_rate)
        
        # Enable denoise and AGC
        val = ctypes.c_int(1)
        self.speex.speex_preprocess_ctl(self.prep_state, 0, ctypes.byref(val))  # DENOISE
    ...
    def _setup_prototypes(self):
        ...
        self.speex.speex_echo_ctl.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p]
        ...
        self.speex.speex_preprocess_ctl.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p]
        
        self.speex.speex_preprocess_run.restype = ctypes.c_int
        self.speex.speex_preprocess_run.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_short)]
...
class AECNode(Node):
    ...
    def __init__(self):
        ...
        # Get parameters
        frame_size = self.get_parameter("frame_size").get_parameter_value().integer_value
        filter_length = self.get_parameter("filter_length").get_parameter_value().integer_value
        sample_rate = self.get_parameter("sample_rate").get_parameter_value().integer_value
        raw_audio_topic = self.get_parameter("raw_audio_topic").get_parameter_value().string_value
        far_audio_topic = self.get_parameter("far_audio_topic").get_parameter_value().string_value
        clean_audio_topic = self.get_parameter("clean_audio_topic").get_parameter_value().string_value
        speaker_state_topic = self.get_parameter("speaker_state_topic").get_parameter_value().string_value
        buffer_size = self.get_parameter("buffer_size").get_parameter_value().integer_value
        self.interrupt_enabled = self.get_parameter("interrupt_enabled").get_parameter_value().bool_value
        ...
        self.clean_audio_publisher = self.create_publisher(AudioData, clean_audio_topic, 20)
        self.interrupt_publisher = self.create_publisher(String, "/speech/interrupt", 10)
        
        self.create_subscription(AudioData, raw_audio_topic, self.near_audio_callback, 20)
        self.create_subscription(AudioData, far_audio_topic, self.far_audio_callback, 20)
        self.create_subscription(Bool, speaker_state_topic, self.speaker_state_callback, 10)
