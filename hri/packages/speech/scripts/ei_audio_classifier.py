...
class EIAudioClassifier(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "AUDIO_SAVE_DIR",
            "/workspace/src/hri/packages/speech/assets/event_detections",
        )
        ...
        self.noise_threshold = (
            self.get_parameter("NOISE_THRESHOLD").get_parameter_value().double_value
        )
    ...
    def classify(self, audio_window: np.ndarray):
        ...
        if self.save_detection_audio:
            self.get_logger().info(
                f"Saving detected audio: {best_label} ({best_score:.2f})"
            )
