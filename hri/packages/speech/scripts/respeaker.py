...
class Respeaker(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "dsp.agc.max_gain_factor", 31.6
        )  # ~30 dB since 20*log10(31.6)≈30
        ...
        self.declare_parameter(
            "dsp.hpf.mode", 2
        )  # HPFONOFF: 0=OFF,1=70Hz,2=125Hz,3=180Hz
    ...
    def callback_light(self, data):
        ...
        if command in {"off", "think", "speak"}:
            pixel_ring.off()
        elif command == "listen":
            pixel_ring.listen()
        else:
            self.get_logger().warn(f"Command: {command} not supported")

    def dsp_setting(self):
        # For proper AEC functionality, audio should be streamed to the speaker and the respeaker.
        try:
            # AGC
            ...
            self.tuning.write(
                "AGCDESIREDLEVEL",
                float(self.get_parameter("dsp.agc.desired_level_dbov").value),
            )
            self.tuning.write(
                "AGCMAXGAIN",
                float(self.get_parameter("dsp.agc.max_gain_factor").value),
            )
            self.tuning.write(
                "AGCTIME", float(self.get_parameter("dsp.agc.time_constant_s").value)
            )

            # Noise suppression
            self.tuning.write(
                "STATNOISEONOFF",
                1 if self.get_parameter("dsp.ns.stationary_on").value else 0,
            )
            self.tuning.write(
                "NONSTATNOISEONOFF",
                1 if self.get_parameter("dsp.ns.nonstationary_on").value else 0,
            )
            ...
            self.tuning.write(
                "GAMMA_E", float(self.get_parameter("dsp.aec.gamma_e").value)
            )
            self.tuning.write(
                "GAMMA_ETAIL", float(self.get_parameter("dsp.aec.gamma_etail").value)
            )
            self.tuning.write(
                "GAMMA_ENL", float(self.get_parameter("dsp.aec.gamma_enl").value)
            )
