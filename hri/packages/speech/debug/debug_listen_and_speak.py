#!/usr/bin/env python3
"""Debug helper (moved): subscribe to processed audio topic, transcribe and speak result.

This script was moved to `speech/debug/` so it's easier to find debug helpers.

It buffers a short segment, writes a temp WAV, attempts to transcribe using the
local faster-whisper transcriber (if available), and then speaks the recognized
text using `gTTS` + `pygame` (fallback prints). Designed for quick local tests.

Usage (from repo root):
  python3 hri/packages/speech/debug/debug_listen_and_speak.py --topic /processedAudioChunk --buffer 3

Notes:
- Install runtime deps (see `debug/requirements.txt`). `rclpy` is provided by ROS 2 and
  should not normally be installed with pip.
"""
...
def try_transcribe_with_faster_whisper(
    ...
    # Attempt to import the local transcriber from scripts/stt.
    try:
        # scripts/stt is relative to speech package
        ...
        # Instantiate a lightweight config; this may download models or raise if unavailable
        model = WhisperModel(
