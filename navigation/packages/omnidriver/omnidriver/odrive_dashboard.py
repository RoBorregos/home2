        ...
        try:
            ...
            if 'N0' not in data:
                ...
                if 'ODOM_phi' in data:
                    ...
                    # The fast/slim line carries the full ODOM_* block (fields
                    # 20..38 -- see the field-index comment above), but until
                    # now this branch only refreshed the web-dashboard cache
                    # above and returned, never touching /odrive/odom or
                    # /odrive/imu -- those were only republished on the rare
                    # 'fat' (N0-present) line. EKF/slam_toolbox need continuous
                    # odometry, not an occasional burst, so republish here too.
                    o_phi = telem['odom_phi']; o_x = telem['odom_x']; o_y = telem['odom_y']
                    o_z   = f(data.get('ODOM_z'))
                    o_w   = telem['odom_w']; o_vx = telem['odom_vx']; o_vy = telem['odom_vy']
                    ...
                    # sensor_msgs/Imu -- the slim line doesn't carry the raw
                    # quaternion/accel fields (those are fat-line-only), so
                    # only orientation (yaw-derived) and yaw rate are real;
                    # that's exactly what the EKF's imu0_config fuses anyway.
                    imu_msg = Imu()
                    imu_msg.header.stamp = now_stamp
                    imu_msg.header.frame_id = self.imu_frame_id
                    imu_msg.orientation = _yaw_to_quaternion(o_phi)
                    imu_msg.angular_velocity.z = telem.get('imu_wz', 0.0)
                    ...
                    if ekf_cov_present:
                        ...
                        _pose_cov[21] = _LARGE
                        _pose_cov[28] = _LARGE
                        _pose_cov[35] = o_var_yaw
                        _twist_cov[0]  = o_var_vx
                        _twist_cov[7]  = o_var_vy
                        _twist_cov[14] = _LARGE
                        _twist_cov[21] = _LARGE
                        _twist_cov[28] = _LARGE
                        _twist_cov[35] = o_var_wz
                    else:
                        _pose_cov[0] = _pose_cov[7] = 0.05
                        _pose_cov[35] = 0.10
                        _twist_cov[0] = 0.02
                        _twist_cov[7] = 0.08
                        _twist_cov[35] = 0.05
