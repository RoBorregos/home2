...
def launch_function(context, *args, **kwargs):
    ...
    rtabmap_map_name = LaunchConfiguration('map_name', default='lab_23_march.db')
    ...
    nav_manager_node = LifecycleNode(
        ...
        namespace='',
        output='screen',
        parameters=[{'managed_nodes': ['map_service'], 'start_parameter_event_publisher': False}],
    )
    ...
    map_context_node = LifecycleNode(
        ...
        name='map_service',
        namespace='',
        parameters=[{'map_name': map_name_str[:-3], 'start_parameter_event_publisher': False}],
        output='screen',
    )
