...
if [ "$BUILD" == "true" ]; then
    SETUP="$SOURCE_ROS && $SOURCE_RTABMAP && $SOURCE_CONSTANTS && $SOURCE_INTERFACES && $CYCLONE_SOURCE && $COLCON && $SOURCE"
else
    SETUP="$SOURCE_ROS && $SOURCE_RTABMAP && $SOURCE_CONSTANTS && $SOURCE_INTERFACES && $SOURCE && $CYCLONE_SOURCE"
fi
