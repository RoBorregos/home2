...
if [ "$PUSH_IMAGE" = true ]; then
  # Navigation uses 'gpu' locally but 'cuda' for image names
  ENV_SUFFIX="$ENV_TYPE"
  [ "$ENV_TYPE" = "gpu" ] && ENV_SUFFIX="cuda"
  push_area_image "navigation" "$ENV_SUFFIX" "$IMAGE_VERSION" "$SCRIPT_DIR"
  exit $?
fi
...
if [ "$PULL_IMAGE" = true ]; then
  ...
  ENV_SUFFIX="$ENV_TYPE"
  [ "$ENV_TYPE" = "gpu" ] && ENV_SUFFIX="cuda"
  pull_area_image "navigation" "$ENV_SUFFIX" "$IMAGE_VERSION"
  exit $?
fi
