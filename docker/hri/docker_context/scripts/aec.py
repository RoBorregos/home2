...
class SpeexAEC:
    ...
    def __init__(self, frame_size, filter_length, sample_rate=16000):
        # Load SpeexDSP library
        library_paths = [
            "/usr/lib/x86_64-linux-gnu/libspeexdsp.so",  # Ubuntu/Debian (Docker)
            "/usr/lib/libspeexdsp.so",                  # Linux
            "/usr/local/lib/libspeexdsp.so",            # Linux (local install)
        ]
        ...
        if speex_lib is None:
            raise RuntimeError(
                "SpeexDSP library not found. Please ensure it is installed in the Docker container."
            )
        ...
        self.frame_size = frame_size
        self.state = self.speex.speex_echo_state_init(frame_size, filter_length)
        self.speex.speex_echo_ctl(self.state, 24, ctypes.byref(ctypes.c_int(sample_rate)))
    ...
    def __del__(self):
        ...
        if hasattr(self, "state"):
            self.speex.speex_echo_state_destroy(self.state)
        if hasattr(self, "prep_state"):
            self.speex.speex_preprocess_state_destroy(self.prep_state)