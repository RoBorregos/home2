...
COLCON="colcon build --symlink-install --packages-up-to manipulation_general vamp_moveit_plugin --packages-ignore realsense_gazebo_plugin xarm_gazebo frida_interfaces task_manager"

if [ "$BUILD" == "true" ]; then
    SETUP="$GPD_SETUP && $VAMP_SETUP && $GPD_EXPORT && $SOURCE_ROS && $SOURCE_INTERFACES && $COLCON && $SOURCE"
else
    SETUP="$GPD_SETUP && $VAMP_SETUP && $GPD_EXPORT && $SOURCE_ROS && $SOURCE_INTERFACES && $SOURCE"
fi
