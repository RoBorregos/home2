...
class VisionTasks:
    ...
    def detect_person(self, timeout: float = TIMEOUT) -> int:
        ...
        # The server polls for up to request.timeout before answering, so give
        # the response a margin over that deadline.
        err, result = self._call(
            self.detect_person_client, request, timeout + 2.0, name="detect_person"
        )
        if err is not None:
            return err
        if not result.success:
