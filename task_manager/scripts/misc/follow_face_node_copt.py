...
class FollowFaceNode(Node):
    ...
    def __init__(self):
        ...
        self.get_joints_client = self.create_client(
            GetJoints, GET_JOINT_SERVICE, callback_group=self.callback_group
        )
        self.joint_states_topic = "/joint_states"
        # create listener
        self.joint_states_sub = self.create_subscription(
            JointState, self.joint_states_topic, self.joint_states_callback, qos_profile
        )
        # change
        ...
        self.move_client = self.create_client(
            MoveVelocity, XARM_MOVEVELOCITY_SERVICE, callback_group=self.callback_group
        )
        ...
        self.create_timer(0.1, self.run, callback_group=self.callback_group)
    ...
    def follow_face_callback(self, request: FollowFace.Request, response: FollowFace.Response):
        ...
        Logger.info(self, f"Follow face active: {self.is_following_face_active}")
    ...
    def move_to(self, x: float, y: float):
        # Use joint trajectory controller with velocity commands
        if not self.is_following_face_active:
            return
        Logger.info(self, f"Moving to X: {x} Y: {y}")
        # Prepare velocity command for joint trajectory controller
        traj_msg = JointTrajectory()
        point = JointTrajectoryPoint()
        # Map x, y to joint velocities (example: joint1 and joint5)
        # You may need to adjust which joints are controlled based on your robot
        x = x * -1
        if x > 0.1:
            x_vel = 0.1
        elif x < -0.1:
        ...
        print(f"Joint positions: {point.positions}")
        point.time_from_start.sec = 0
        point.time_from_start.nanosec = int(2e8)  # 0.2s
...
def main(args=None):
    ...
    executor.add_node(demo_task_manager)
    try:
        executor.spin()
    finally:
