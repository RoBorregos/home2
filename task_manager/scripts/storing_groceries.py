...
class StoringGroceriesTM(Node):
    ...
    def apply_penalty(self, penalty_type):
        ...
        if penalty_type in self.penalties and self.penalties[penalty_type] < 5:
            ...
            Logger.info(
                self,
                f"Applied {penalty_type} penalty: {penalty_value}. Total score: {self.total_score}",
            )
            return True
    ...
    def request_human_handover(self):
        ...
        if self.apply_penalty("HUMAN_HANDOVER"):
            self.subtask_manager.hri.say(
                "I'm having trouble picking up the object. Could you please hand it to me?"
            )
    ...
    def request_human_pointing(self):
        ...
        if self.apply_penalty("HUMAN_POINTING"):
            self.subtask_manager.hri.say(
                "I'm having trouble determining where to place this object. Could you point to the correct location?"
            )
    ...
    def request_open_cabinet(self, door_number):
        ...
        if self.apply_penalty("HUMAN_OPEN_CABINET"):
            self.subtask_manager.hri.say(
                f"I'm having trouble opening cabinet door {door_number}. Could you help open it?"
            )

            # Wait for confirmation of door opening
            while not self.subtask_manager.hri.confirm(
                "Have you opened the cabinet door?", timeout=30
            ):
            ...
            self.subtask_manager.hri.say(
                "I've already received maximum assistance with opening cabinets."
            )
            return Status.EXECUTION_FAILURE
    ...
    def handle_cereal_spill(self):
        ...
        self.subtask_manager.hri.say(
            "I apologize for spilling some cereal. I will be more careful."
        )
    ...
    def handle_leftover_cereal(self):
        ...
        self.subtask_manager.hri.say(
            "I notice there is cereal left in the box. I should pour all of it."
        )
    ...
    def request_human_pour_cereal(self):
        ...
        if self.apply_penalty("HUMAN_POUR_CEREAL"):
            self.subtask_manager.hri.say(
                "I'm having significant difficulty pouring the cereal. Could you please pour it for me?"
            )
        ...
        if self.current_state == StoringGroceriesTM.TASK_STATES["START"]:
            ...
            if self.navigate_to("table") == Status.EXECUTION_SUCCESS:
                ...
                if self.current_attempts >= ATTEMPT_LIMIT:
                    self.subtask_manager.hri.say(
                        "I couldn't reach the table after multiple attempts. Ending task."
                    )
            ...
            if status == Status.EXECUTION_SUCCESS and objects:
                ...
                self.subtask_manager.hri.say(
                    f"I have detected {len(objects)} grocery items on the table."
                )
                ...
                if self.current_attempts >= ATTEMPT_LIMIT:
                    self.subtask_manager.hri.say(
                        "I couldn't detect any groceries on the table. Moving on."
                    )
                    self.current_state = StoringGroceriesTM.TASK_STATES["END"]
                else:
            ...
            if not self.objects_to_organize:
                ...
                if self.cereal_found and self.cereal_container_found:
                    self.subtask_manager.hri.say(
                        "I've stored all groceries from the table. Now I'll pour cereal."
                    )
            ...
            self.subtask_manager.hri.say(
                f"I see a {self.current_object['label']}. It belongs to category: {self.current_object_category}"
            )
            ...
            if status == Status.EXECUTION_SUCCESS:
                self.subtask_manager.hri.say(
                    f"I have picked up the {self.current_object['label']}."
                )
                ...
                if self.current_attempts >= ATTEMPT_LIMIT:
                    ...
                    if self.request_human_handover() == Status.EXECUTION_SUCCESS:
                        self.subtask_manager.hri.say(
                            f"Now I have the {self.current_object['label']}."
                        )
                        self.current_state = StoringGroceriesTM.TASK_STATES["NAVIGATE_TO_CABINET"]
                    else:
                        self.subtask_manager.hri.say(
                            "Skipping this grocery item due to difficulties."
                        )
                        # Reset attempts for next object
                        self.current_attempts = 0
                else:
                    self.subtask_manager.hri.say(
                        f"Failed to pick up the {self.current_object['label']}. Trying again."
                    )
            ...
            if self.navigate_to("cabinet") == Status.EXECUTION_SUCCESS:
                ...
                self.subtask_manager.hri.say(
                    "Failed to reach the cabinet. Will try to place the grocery item anyway."
                )
                self.current_state = StoringGroceriesTM.TASK_STATES["PERCEIVE_CABINET_OBJECTS"]
            ...
            self.subtask_manager.hri.say(
                "I will now scan the cabinet to understand its organization."
            )
            ...
            if status == Status.EXECUTION_SUCCESS:
                ...
                self.subtask_manager.hri.say(
                    "I'm having trouble scanning the cabinet. I'll make my best guess for placement."
                )
                self.current_state = StoringGroceriesTM.TASK_STATES["DETERMINE_PLACEMENT_LAYER"]
            ...
            if self.current_object_category in self.cabinet_layers:
                ...
                self.subtask_manager.hri.say(
                    f"This {self.current_object['label']} should go on the {target_layer} layer with other {self.current_object_category} items."
                )
            else:
                # Determine based on predefined rules
                target_layer = self.determine_cabinet_layer(self.current_object_category)
                self.subtask_manager.hri.say(
                    f"This {self.current_object['label']} belongs to {self.current_object_category} category and should go on the {target_layer} layer."
                )
            ...
            if similar_objects:
                self.subtask_manager.hri.say(
                    f"I'll place this {self.current_object['label']} next to other {self.current_object_category} items."
                )
                # Enhanced placement next to similar objects
                placement_status = self.subtask_manager.manipulation.place_next_to_similar(
                    target_position, self.current_object_category
                )
            else:
                self.subtask_manager.hri.say(
                    f"I'll place this {self.current_object['label']} on the {target_layer} layer."
                )
                placement_status = self.subtask_manager.manipulation.place(target_position)

            if placement_status == Status.EXECUTION_SUCCESS:
                self.subtask_manager.hri.say(
                    f"I have placed the {self.current_object['label']} successfully."
                )
                ...
                if self.current_attempts >= ATTEMPT_LIMIT:
                    ...
                    self.subtask_manager.hri.say(
                        f"Having trouble placing the {self.current_object['label']}. Trying again."
                    )

        elif self.current_state == StoringGroceriesTM.TASK_STATES["OPEN_FIRST_CABINET"]:
            Logger.state(self, "Opening first cabinet door")
            self.subtask_manager.hri.say(
                "I will now open the first cabinet door to access the cereal storage area."
            )
            ...
            if status == Status.EXECUTION_SUCCESS:
                ...
                if self.current_attempts >= ATTEMPT_LIMIT:
                    ...
                    self.subtask_manager.hri.say(
                        "Failed to open the second cabinet door. Trying again."
                    )
            ...
            if container_object:
                ...
                pour_status = self.subtask_manager.manipulation.pour(
                    source_object=cereal_object, target_object=container_object
                )
