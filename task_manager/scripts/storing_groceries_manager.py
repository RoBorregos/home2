        ...
        if self.state == ExecutionStates.START:
            ...
            if len([i for k, i in self.object_to_placing_shelf.values() if k == 0]) == 0:
                status, res = self.subtask_manager.hri.confirm(
                    text="Did I pick all the objects?", use_wakewords=False
                )
                if res == "yes":
                    self.subtask_manager.hri.say(
            ...
            try:
                ...
                # )
                # self.subtask_manager.hri.confirm("Can you please open the shelve cabinet for me? Please say yes when done!", use_wakewords=True)
                hres: Status = self.nav_to("kitchen", "table")
                hres = Status.EXECUTION_SUCCESS
                time.sleep(2)
                self.subtask_manager.hri.say(text="Finding Table", wait=False)
            ...
            while (
                self.subtask_manager.hri.confirm(
                    "Have you handed me the object?", use_wakewords=False
                )[1]
                != "yes"
            ):
            ...
            if (
                ...
                while (
                    self.subtask_manager.hri.confirm(
                        "Have you grabbed the object?", use_wakewords=False
                    )[1]
                    != "yes"
                ):
            ...
            self.subtask_manager.hri.confirm(
                "Have you grabbed the object?", use_wakewords=False, retries=5
            )
            self.subtask_manager.manipulation.open_gripper()
            while (
                self.subtask_manager.hri.confirm(
                    "Have you placed the object?", use_wakewords=False
                )[1]
                != "yes"
            ):
