...
BASE_VX_CAP = 0.5              # nav2_omni_following.yaml vx_max
...
class FollowCalibration(Node):
    ...
    def summary(self):
        ...
        if self.cmd_ticks:
            print(f"  base velocity    : max {self.vx_max_seen:.2f} m/s (cap {BASE_VX_CAP})   "
        ...
        if self.centroid.count == 0:
            ...
            if self.cmd_ticks and self.vx_sat_ticks / self.cmd_ticks > 0.5:
                hints.append(
                    ...
                    "the base — raise vx/vy in nav2_omni_following.yaml (test safety!) "
                    "or accept larger follow_distance."
                )
        if not hints:
