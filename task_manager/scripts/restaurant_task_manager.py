    ...
    def run(self):
        ...
        if self.current_state == RestaurantTaskManager.TaskStates.WAIT_FOR_CALL:
            ...
            if status == Status.EXECUTION_SUCCESS and person_point.header.frame_id != "":
                ...
                self.subtask_manager.hri.say("I will try to find a customer again.", wait=False)
            else:
            ...
            if status == Status.EXECUTION_SUCCESS:
                ...
                if re_status == Status.EXECUTION_SUCCESS and re_point.header.frame_id != "":
                    ...
                    if (
                        ...
                        Logger.info(
                            self, f"Customer still {camera_dist:.2f}m away, approaching again..."
                        )
            ...
            if self.current_customer_index < table["num_customers"]:
                ...
                if self.current_customer_index == 0:
                    ...
                    status, _ = self.subtask_manager.nav.move_to_point(
                        point=table["coordinates"], standoff_distance=0.5
                    )
                    if status == Status.EXECUTION_SUCCESS:
            ...
            if not self._orders_given:
                ...
                for idx, tid in enumerate(self.tables_sorted_by_customers):
                    ...
                    if orders:
                        ...
                        table_breakdowns.append(f"for table {idx + 1}: {', '.join(orders)}")
