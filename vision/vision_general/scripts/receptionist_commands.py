...
class ReceptionistCommands(Node):
    def __init__(self):
        ...
        self.find_seat_service = self.create_service(FindSeat, FIND_SEAT_TOPIC, self.find_seat_callback)
    ...
    def detect_person(self):
        ...
        for out in results:
            for box in out.boxes:
                ...
                if confidence > CONF_THRESHOLD and x >= int(width*PERCENTAGE) and x <= int(width*(1-PERCENTAGE)):
                    self.person_found = True
                    cv2.rectangle(self.output_image, (int(x - w / 2), int(y - h / 2)),
    ...
    def get_detections(self, frame) -> None:
        ...
        for out in results:
            for box in out.boxes:
                ...
                if confidence < CONF_THRESHOLD or area_percentage < AREA_PERCENTAGE_THRESHOLD:
                    continue
    ...
    def check_chairs(self, frame) -> tuple[bool, float]:
        ...
        for chair in self.chairs:
            ...
            cv2.circle(self.output_image, (int((xmax-xmin)/2), int(y_center_chair)), 5, (0, 255, 255), -1)
            ...
            for person in self.people:
                ...
                if center_x >= xmin and center_x <= xmax and person_y > y_center_chair:
                    occupied = True
                    cv2.circle(self.output_image, (int(center_x), int(person_y)), 5, (0, 0, 255), -1)
                    break

            if not occupied:
                area = xmax - xmin
                output = (chair["bbox"][0] + chair["bbox"][2]) / 2
                chair_queue.put((-1*area, output, chair["bbox"][0],chair["bbox"][1],chair["bbox"][2],chair["bbox"][3]))
            else:
                cv2.rectangle(self.output_image, (xmin, chair["bbox"][1]), (xmax, chair["bbox"][3]), (0, 0, 255), 2)
    ...
    def check_couches(self, frame) -> tuple[bool, float]:
        ...
        if available_spaces.qsize() > 0:
            ...
            output = (left + right) / 2
            cv2.rectangle(self.output_image, (left, 0), (right, frame.shape[0]), (0, 255, 0), 2)
