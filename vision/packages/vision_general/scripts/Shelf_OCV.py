        ...
        for i in range(0, len(linesP)):
            ...
            if dy <10:  # Adjust the threshold as needed
                ...
                for j in range(len(merged_horizontal_lines)):
                    ...
                    if abs(line_segment[1] - ml[1]) < THRESHOLD_MIN_HORIZONTAL and (abs(line_segment[0] - ml[2]) < THRESHOLD_MIN_HORIZONTAL or abs(line_segment[2] - ml[0]) < 100):
                        merged_horizontal_lines[j] = [min(ml[0], line_segment[0]), ml[1], max(ml[2], line_segment[2]), ml[3]]
                        merged = True
                        break
                if not merged:
        ...
        for i in range(0, len(linesP)):
            ...
            if dx < 50:  # Adjust the threshold as needed
                ...
                for j in range(len(merged_vertical_lines)):
                    ...
                    if abs(line_segment[0] - ml[0]) < THRESHOLD_MIN_VERTICAL and (abs(line_segment[1] - ml[3]) < THRESHOLD_MIN_VERTICAL or abs(line_segment[3] - ml[1]) < THRESHOLD_MAX_Y):
                        merged_vertical_lines[j] = [ml[0], min(ml[1], line_segment[1]), ml[2], max(ml[3], line_segment[3])]
                        merged = True
                        break
                if not merged:
