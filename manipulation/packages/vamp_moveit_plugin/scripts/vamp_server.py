...
class VampServer(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "VAMP server for FRIDA is ready and waiting for planning requests"
        )
    ...
    def plan_callback(self, request, response):
        ...
        try:
            ...
            for i in range(len(radii)):
                ...
                self.get_logger().info(
                    f"  Sphere {i+1}: center=[{pos[0]:.4f}, {pos[1]:.4f}, {pos[2]:.4f}], radius={radii[i]:.4f}"
                )
            ...
            for i in range(num_boxes):
                ...
                self.get_logger().info(
                    f"  Box {i+1}: size={lx:.3f}x{ly:.3f}x{lz:.3f}, approximating with {nx*ny*nz} spheres"
                )
                ...
                for dx in np.linspace(-lx / 2.0, lx / 2.0, nx):
                    for dy in np.linspace(-ly / 2.0, ly / 2.0, ny):
                        for dz in np.linspace(-lz / 2.0, lz / 2.0, nz):
                            ...
                            env.add_sphere(
                                vamp.Sphere(sphere_center, float(sphere_radius))
                            )
            finger_left = 0.8
            finger_right = 0.8
            start_8 = np.array(
                list(request.start_state) + [finger_left, finger_right],
                dtype=np.float64,
            )
            goal_8 = np.array(
                list(request.goal_state) + [finger_left, finger_right], dtype=np.float64
            )
            ...
            if not is_start_valid:
                self.get_logger().error(
                    "Start state is in collision with environment or self-colliding"
                )
            if not is_goal_valid:
                self.get_logger().error(
                    "Goal state is in collision with environment or self-colliding"
                )

            if not is_start_valid or not is_goal_valid:
                clean_env = vamp.Environment()
                if vamp.frida_real.validate(
                    start_8, clean_env
                ) and vamp.frida_real.validate(goal_8, clean_env):
                    self.get_logger().info(
                        "Note: Both states are valid without obstacles. The collision is likely due to self-collision or joint limits"
                    )
                else:
                    self.get_logger().error(
                        "States are invalid even without obstacles. Check joint values and ensure they're within limits and not self-colliding"
                    )
            ...
            self.get_logger().info(
                "Start and goal states validated successfully. Starting RRT-Connect planner..."
            )

            self.get_logger().info(
                "Initializing Halton sequence generator for sampling..."
            )
            try:
                rng = vamp.frida_real.halton()
                self.get_logger().info(
                    "Halton generator initialized successfully with default settings."
                )
            except TypeError:
                try:
                    rng = vamp.frida_real.halton(6)
                    self.get_logger().info(
                        "Halton generator initialized successfully with 6 dimensions."
                    )
            ...
            if result and result.solved:
                ...
                try:
                    ...
                    self.get_logger().warn(
                        f"Could not simplify path, using original trajectory: {e}"
                    )

                n_points = len(raw_path)
                self.get_logger().info(
                    f"Path planning success. Validating {n_points} waypoints..."
                )
                ...
                for i in range(n_points - 1):
                    ...
                    for t in np.linspace(0, 1, num_steps, endpoint=False):
                        ...
                        if not vamp.frida_real.validate(interp_wp, env):
                            self.get_logger().error(
                                f"Collision detected during path interpolation at segment {i}"
                            )
                            path_is_safe = False
                            break
                ...
                if path_is_safe:
                    ...
                    final_distance = np.linalg.norm(
                        dense_path[-1][:6] - clean_points[-1][:6]
                    )
                    ...
                    self.get_logger().info(
                        f"Success! Path refined to {len(clean_points)} key waypoints for smooth motion"
                    )
                else:
                    self.get_logger().warn(
                        "Path validation failed due to internal collision."
                    )
                    response.success = False
            else:
