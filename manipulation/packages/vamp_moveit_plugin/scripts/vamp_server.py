...
ARM_DOF = 6
ARM_JOINT_NAMES = [f"joint{i}" for i in range(1, ARM_DOF + 1)]


def load_joint_limits_from_urdf(urdf_path, joint_names):
    """Parse a URDF file and return [(lower, upper), ...] for the named joints,
    in the order given. Raises if any joint or its <limit> tag is missing."""
    ...
    for name in joint_names:
        ...
        lim = j.find("limit")
        if lim is None:
            raise ValueError(f"Joint '{name}' has no <limit> tag in URDF {urdf_path}")
...
class VampServer(Node):
    def __init__(self):
        ...
        # Path to the URDF whose joint limits should gate planning requests.
        # Defaults to the spherized URDF shipped with this package — the same
        # one frida_real's collision model was generated from, so the limits
        # stay in sync with VAMP's geometric model by construction.
        default_urdf = os.path.join(
            get_package_share_directory("vamp_moveit_plugin"),
            "resources",
            "xarm6_spherized.urdf",
        )
        self.declare_parameter("joint_limits_urdf", default_urdf)
        ...
        urdf_path = self.get_parameter("joint_limits_urdf").value
        try:
            self.joint_limits = load_joint_limits_from_urdf(urdf_path, ARM_JOINT_NAMES)
            limit_str = ", ".join(
                f"{n}:[{lo:.3f},{hi:.3f}]"
                for n, (lo, hi) in zip(ARM_JOINT_NAMES, self.joint_limits)
            )
            self.get_logger().info(f"Loaded joint limits from {urdf_path}: {limit_str}")
        except Exception as e:
            self.get_logger().fatal(
                f"Failed to load joint limits from URDF '{urdf_path}': {e}. "
                "Refusing to start — fix the joint_limits_urdf parameter."
            )
            raise
    ...
    def to_vamp(joint_values):
        return np.array(joint_values[:ARM_DOF], dtype=np.float32)

    @staticmethod
    def from_vamp(joint_values):
        return [float(x) for x in joint_values[:ARM_DOF]]
    ...
    def build_environment(self, request, start_config=None, goal_config=None):
        ...
        if len(radii) > 0:
            ...
            r_point = max(
                max(avg_radius, self.security_margin) + self.security_margin,
                self.min_r_point,
            )
            n_raw = len(pc)
            ...
            if len(pc) > 0:
                try:
                    ...
                    self.get_logger().warn(
                        f"  CAPT failed: {e}, falling back to spheres"
                    )
                    for pt in pc:
        ...
        for i in range(num_boxes):
            ...
            try:
                self.get_logger().info(
                    f"  Cuboid: center={center}, half={half_extents}"
                )
    ...
    def _check_joint_limits(self, label, state):
        """Return a diagnostic string if any joint is out of URDF limits, else None."""
        violations = []
        for i, (lo, hi) in enumerate(self.joint_limits):
            v = float(state[i])
            if v < lo or v > hi:
                violations.append(
                    f"{ARM_JOINT_NAMES[i]}={v:.3f} outside [{lo:.3f}, {hi:.3f}]"
                )
        if violations:
            return f"{label} INVALID (joint-limits: {'; '.join(violations)})"
        return None

    def validate_states(self, start, goal, env):
        # Joint-limit gate first: vamp.frida_real.validate() only checks
        # geometric self-collision, not joint limits. Without this, out-of-range
        # configs reach RRTConnect, which then wastes all retry budget producing
        # an unhelpful "planning failed" instead of "joint2 is out of range".
        limit_diag = self._check_joint_limits("Start", start)
        if limit_diag:
            return limit_diag
        limit_diag = self._check_joint_limits("Goal", goal)
        if limit_diag:
            return limit_diag
        ...
        if not is_start_valid:
            cause = "self-collision" if not start_clean else "environment collision"
            msgs.append(f"Start INVALID ({cause})")
        if not is_goal_valid:
            cause = "self-collision" if not goal_clean else "environment collision"
    ...
    def plan_callback(self, request, response):
        ...
        try:
            ...
            self.get_logger().info(
                f"  Radii: {len(request.sphere_radii)}, Boxes: {len(request.box_sizes_flat) // 3}"
            )
            ...
            for attempt in range(self.max_retries):
                ...
                settings.max_iterations = int(
                    settings.max_iterations * self.retry_iter_mult
                )
            ...
            with self.Timer("Path simplify (SHORTCUT+BSPLINE)", self.get_logger()):
                try:
                    ...
                    if simplified.solved and len(simplified.path) > 0:
                        ...
                        self.get_logger().info(
                            f"  Native simplify: {len(path_nodes)} waypoints"
                        )
                except Exception as e:
            ...
            if path_nodes is None:
                ...
                path_nodes = [
                    np.array(list(raw_path[int(i)]), dtype=np.float32)
            ...
            with self.Timer("Final validation & Densify", self.get_logger()):
                final_dense_path, is_safe = self.validate_and_densify_path(
                    path_nodes, env
                )
            if not is_safe:
                self.get_logger().warn(
                    "Path has collision, retrying without smoothing."
                )
                n_points = len(raw_path)
                step = max(1, n_points // 500)
                path_nodes = [
