...
def launch_setup(context, *args, **kwargs):
    ...
    moveit_config_dict = (
        yaml.load(moveit_config_dump, Loader=yaml.FullLoader)
        if moveit_config_dump
        else {}
    )
        ...
        robot_description = {
            "robot_description": get_xacro_file_content(
                xacro_file=PathJoinSubstitution(
                    [FindPackageShare("frida_description"), "urdf", "FRIDA.urdf.xacro"]
                ),
                arguments={
                    "prefix": prefix,
                    "dof": dof,
    ...
    xarm_gazebo_world = PathJoinSubstitution(
        [FindPackageShare("arm_pkg"), "worlds", "empty.world"]
    )
    ...
    gazebo_spawn_entity_node = Node(
        ...
        arguments=[
            ...
            "0",
            "-y",
            "0" if robot_type.perform(context) == "uf850" else "-0.5",
            "-z",
            "0.2",
            "-Y",
            "0",
        ...
        parameters=[
            {
                ...
                "robot_description_semantic": moveit_config_dict.get(
                    "robot_description_semantic", ""
                ),
                "robot_description_kinematics": moveit_config_dict.get(
                    "robot_description_kinematics", {}
                ),
                "robot_description_planning": moveit_config_dict.get(
