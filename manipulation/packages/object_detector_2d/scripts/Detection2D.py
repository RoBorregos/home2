...
class CamaraProcessing:
    def __init__(self):
        # Initialize ROS node
        rclpy.init()
        self.node = rclpy.create_node("detection2d")
        ...
        self.handleSource()
        self.publisher = rclpy.Publisher(
    ...
    def compute_result(self, frame):
        ...
        detections["detection_boxes"] = (
            np.array(
                [
                    [1 - box[2], 1 - box[3], 1 - box[0], 1 - box[1]]
                    for box in visual_detections["detection_boxes"]
                ]
            )
            if ARGS["FLIP_IMAGE"]
            else visual_detections["detection_boxes"]
        )
        return (
            self.get_objects(
                detections["detection_boxes"],
        ...
        for index, value in enumerate(classes):
            if scores[index] > ARGS["MIN_SCORE_THRESH"]:
                ...
                point3d = PointStamped(
                    header=Header(frame_id=ARGS["CAMERA_FRAME"]), point=Point()
                )
