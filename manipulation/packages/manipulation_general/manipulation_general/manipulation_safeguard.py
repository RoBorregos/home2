...
class ManipulationSafeguard(Node):
    def __init__(self):
        ...
        # service, and so a fault (state=STOPPED or err!=0) broadcasts on ESTOP_TOPIC —
        # the only signal that stops mode-5 cartesian motion (see robot/arm.py).
        ...
        self.get_logger().info(
            "Manipulation Safeguard node started: ensure_arm_ready service only "
            "(no autonomous recovery — call ensure_arm_ready to recover after an e-stop)"
        )
    ...
    def _handle_ensure_arm_ready(self, request, response):
        ...
        if ok and self._in_estop:
            self._in_estop = False
            self.get_logger().warn(
                "E-stop CLEARED — arm recovered via ensure_arm_ready"
            )
