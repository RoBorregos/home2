...
# Add contact_graspnet to sys.path
...
class ContactGraspNetNode(Node):
    def __init__(self):
        ...
        forward_passes = self.get_parameter('forward_passes').get_parameter_value().integer_value
        
        self.get_logger().info(f'Loading model from {ckpt_dir}...')
        
        try:
            self.global_config = config_utils.load_config(ckpt_dir, batch_size=forward_passes)
        ...
        self.srv = self.create_service(GraspDetection, 'detect_grasps', self.detect_grasps_callback)
    ...
    def detect_grasps_callback(self, request, response):
        ...
        try:
            ...
            points_list = []
            for p in pc2.read_points(request.input_cloud, field_names=("x", "y", "z"), skip_nans=True):
        ...
        try:
            ...
            self.get_logger().info(f'Processing cloud with {pc_full.shape[0]} points...')
