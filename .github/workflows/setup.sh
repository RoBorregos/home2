...
# nanobind's stubgen.py (invoked by vamp's CMakeLists during build) needs typing_extensions.TypeVarTuple
# noble ships no python3-typing-extensions at all, and its pip enforces PEP 668, hence --break-system-packages
echo "INSTALL PYTHON BUILD DEPS (modern typing_extensions for vamp/nanobind stubgen)"
sudo apt install -y python3-pip
sudo pip install --break-system-packages --upgrade 'typing_extensions>=4.6'


echo "ROSDEP INIT"
# the ros:jazzy image ships an initialized rosdep, so this is expected to be a no-op there
rosdep init || true
