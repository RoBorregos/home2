...
| **HRI**          | Med (audio I/O, embeddings)                               | **High** when speech/LLM active (STT+TTS+noise-cancel+embeddings, all local; LLM optionally local)            | 5-10 GB with the local 4B LLM; 2-4 GB if the LLM is external          | 4-8 GB                               | 40-80 W local LLM; 15-25 W with API-backed LLM |
| **Integration**  | Very low (1-2 cores: FSM + DDS/iceoryx)                   | None                                                                                                          | 0                                                                     | ~1.5-2 GB (mostly iceoryx SHM pools) | 5-10 W                                         |

**Read this table relatively, not absolutely**: these areas don't all run at full load simultaneously (e.g. GPD only spikes during a pick attempt; RTABMap and the local LLM are both opt-in). Concurrent worst case (everything active at once) plausibly exceeds the Orin AGX's realistic sustained budget — see [Contention Risks](#contention-risks-and-caveats).
...
| LLM (dialogue/reasoning) | Qwen3.5-4B (`UD-Q4_K_XL` GGUF on Jetson; `qwen3.5:4b` via Ollama), **or** external OpenAI-compatible API | **~3-5 GB shared GPU memory if local**, near-zero local compute if API | **Configurable** — biggest swing factor in this area |

On Jetson/L4T, the production path serves `Qwen3.5-4B-UD-Q4_K_XL.gguf` through the NVIDIA `llama.cpp` Jetson image under the stable `frida-llm` alias. Reasoning is disabled at server startup for low latency. The KV cache defaults to `f16` for correctness; set `LLAMA_CACHE_TYPE=q8_0` for the target-hardware A/B quality check before adopting the lower-memory cache. The CUDA development path uses the pinned official Ollama image, mounts the same model store, and maps the explicit `qwen3.5:4b` tag to `frida-llm`.

**This is the area with the widest resource range**, entirely dependent on one config flag (`base_url` for the LLM wrapper). With everything local (STT+TTS+noise-cancel+embeddings+deberta+Qwen3.5-4B), GPU-memory demand is approximately **5-10 GB** — a significant fraction of the Orin AGX's shared memory pool, competing directly with Vision's Moondream2 and detector stack. Routing the LLM to an external API cuts that to **2-4 GB** and removes the single largest local GPU consumer in the whole system.
...
- **GPU/VRAM is the tightest shared resource.** Vision's detector stack (~2 GB) + Moondream2 (~4-5 GB) + HRI's local models (~3-6 GB) can together approach or exceed what's comfortable to share with manipulation/navigation's occasional GPU bursts. If all of Vision + HRI's local-LLM path run concurrently, memory pressure is the most likely failure mode — worth watching if the Orin starts throttling or OOM-killing containers.
...
- **The LLM backend choice in HRI (local runtime vs. external API) is the single biggest lever** for total system resource pressure — it swings HRI's footprint by several GB of GPU memory and tens of watts.
