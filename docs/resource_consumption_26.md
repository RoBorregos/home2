...
Approximate CPU / GPU / VRAM / RAM / energy footprint per software area, based on a static read of the models, frameworks, and launch configuration in this repo (no profiling was run). Target platform: **NVIDIA Jetson Orin AGX** (12-core ARM CPU, ~64GB shared LPDDR5, Ampere GPU, 15–60W configurable power envelope). Numbers are engineering estimates, not benchmarks — treat them as relative sizing, not SLAs.

> Note on staleness: an earlier session's notes described a Contact-GraspNet (CGN) integration replacing GPD in manipulation. A fresh scan of `manipulation/` and full git history on 2026-08-15 found **no trace of CGN** (no `contact_graspnet_ros` package, no submodule, no matching commits). The current pipeline is GPD-only. This doc reflects current code; the CGN work either never landed or was reverted before being committed.
...
| iceoryx (roudi daemon) | ~1-2% CPU, ~700-800 MB RAM (shared memory pools, mostly for camera/depth frame buffers) | Shared across *all* areas, not integration-specific compute — it's the IPC backbone |
