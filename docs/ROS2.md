...
In this example we are using an action file called `ExampleAction` that should be imported from `frida_interfaces` and has the following structure:
...
``` python
    def __init__(self):
        ...
        self.example_action_server = ActionServer(self, ExampleAction, "/example/action_service_topic", self.action_callback)
...
``` bash
ros2 action send_goal /example/action_service_topic frida_interfaces/action/ExampleAction "request: true" --feedback
```
